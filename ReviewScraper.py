...
class SmartStoreReviewScraper:
    ...
    def get_store_data(self, link):
        ...
        if r.status_code == 200:
            ...
            product_no = re.search('[0-9]+',link.split('products/')[1].split('?')[0]).group(0)
            data = {
    ...
    def scrape_review_contents(self, REVIEWS, review_content):
        for review in review_content:
            data = {
                    'rate': review['reviewScore'], #평점
                    # '아이디': review['writerMemberId'], #아이디
                    'date': review['createDate'].replace('T', ' ').split('.')[0].split(' ')[0], #시간   
                }
            data['raw'] = re.sub(r'[@%\\*\:\;\-\_\$\n\t\r\,\~]',' ',review['reviewContent']) # remove special
